package com.rainbow.cloud.client.kafka.logback;
...
import com.rainbow.cloud.base.constant.RpcType;
import com.rainbow.cloud.base.util.StringUtil;
import com.rainbow.cloud.client.constant.KafkaConfig;
import com.rainbow.cloud.client.constant.NodeMode;
import com.rainbow.cloud.client.kafka.SingletonProducer;
import com.rainbow.cloud.client.kafka.logback.encoder.KafkaLayoutEncoder;
import com.rainbow.cloud.client.kafka.partitioner.KeyBuilder;
import com.rainbow.cloud.client.kafka.partitioner.KeyModPartitioner;
import com.rainbow.cloud.client.register.ZkRegister;
import com.rainbow.cloud.client.util.SysUtil;
import com.rainbow.cloud.trace.dto.RegisterDto;
import com.rainbow.cloud.trace.generater.IncrementIdGen;
import com.rainbow.cloud.trace.registry.Registry;
import com.rainbow.cloud.trace.registry.ZookeeperRegistry;
...
public class KafkaAppender<E> extends UnsynchronizedAppenderBase<E>  {
    ...
    public void stop() {
        ...
        // 关闭KafkaProuder
        if (SingletonProducer.isInstanced()) {
            // producer实际上已经初始化
            SingletonProducer.getInstance(this.config).close();
        }
    ...
    protected void append(E e) {
        ...
        SingletonProducer.getInstance(this.config).send(record, new Callback() {
            @Override
